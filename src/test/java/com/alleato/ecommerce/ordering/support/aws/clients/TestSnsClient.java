...
/**
 * Test implementation of the AWS SNS client. Instead of calling real SNS, it:
 *  - Validates that the topic ARN is one of the known/configured topics
 *  - Records all Publish calls for test assertion
 *  - Returns realistic responses
 *  - Throws realistic errors for unknown topic ARNs
 *  - Supports configurable exception rules to simulate runtime failures
 *
 * This allows integration tests to use the real {@link com.alleato.ecommerce.ordering.notification.SnsNotificationClient}
...
public class TestSnsClient implements SnsClient {
    ...
    /**
     * Configure an exception to be thrown when the given condition matches a request.
     * Useful for simulating SNS outages, throttling, or authorization failures.
     */
