...
/**
 * Test implementation of the AWS S3 client. Instead of calling real S3, it:
 *  - Validates that the bucket is one of the known/configured buckets
 *  - Stores objects in-memory
 *  - Records all PutObject calls for test assertion
 *  - Returns realistic responses and errors
 *  - Supports configurable exception rules to simulate runtime failures
 *
 * This allows integration tests to use the real {@link com.alleato.ecommerce.ordering.invoicing.S3DocumentClient}
 * implementation while intercepting the actual AWS API calls.
 */
public class TestS3Client implements S3Client {
    ...
    public PutObjectResponse putObject(PutObjectRequest request, RequestBody requestBody) {
        ...
        return PutObjectResponse.builder()
                .eTag("\"test-etag-" + putRequests.size() + "\"")
                .build();
    }
    ...
    /**
     * Configure an exception to be thrown when a PutObject request matches the condition.
     * Useful for simulating S3 write failures, access denied, etc.
     */
    ...
    /**
     * Configure an exception to be thrown when a GetObject request matches the condition.
     * Useful for simulating S3 read failures, access denied, etc.
     */
