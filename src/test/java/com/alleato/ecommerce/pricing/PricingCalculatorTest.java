 ...
 *
 * PricingCalculator is pure business logic with no external dependencies.
 * Unit tests are the right choice here because:
 *  - The value is in verifying the pricing ALGORITHM (combinations of promos, bulk discounts, caps)
 *  - There's nothing to integrate with — no DB, no HTTP, no external services
 *  - Tests run in milliseconds and cover many edge cases easily
 *
 * This is the "contract" we're testing: given these inputs, the pricing rules produce these outputs.
 * We don't care HOW it computes — just that the contract holds.
 */
