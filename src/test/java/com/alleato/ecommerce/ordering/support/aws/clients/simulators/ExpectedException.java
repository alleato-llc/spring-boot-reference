...
/**
 * Manages configurable exception rules for simulator operations. Rules are checked before an
 * operation proceeds — if any rule's condition matches the request, its exception is thrown.
 *
...
public class ExpectedException<T> {
  ...
  /**
   * Check configured exception rules against the request. Throws the configured exception if any
  ...
  /**
   * Register an exception rule. When {@link #checkRules} encounters a request matching the
   * condition, it throws the supplied exception.
   */
