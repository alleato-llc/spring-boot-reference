 ...
 *
 * No HTTP involved — this records reservation calls and returns configurable
 * responses so integration tests can verify the order workflow correctly
 * interacts with the inventory service.
 *
 * Supports:
 *  - Configuring which products are out of stock
 *  - Configurable exception rules to simulate arbitrary failures
 *  - Recording all reservation calls for assertion
 *  - Returning realistic confirmation responses
 */
public class TestInventoryClient implements InventoryClient {
     ...
     * Useful for simulating service outages, timeouts, or other runtime failures.
     *
