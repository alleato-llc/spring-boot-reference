...
/**
 * INTEGRATION TEST — verifies that Flyway migrations run correctly against
 * a real Postgres instance (via docker-compose).
 *
 * This catches issues that in-memory databases (H2) would miss:
 *  - Postgres-specific syntax (e.g. BIGSERIAL, TIMESTAMP WITH TIME ZONE)
 *  - ALTER TABLE migrations that depend on existing schema
 *  - Index creation on Postgres
 *
 * This is a key benefit of testing against real Postgres: you test against
 * the real database engine, not a substitute.
 */
class FlywayMigrationIntegrationTest extends BaseIntegrationTest {
    ...
    void migrationsProduceExpectedSchema() throws Exception {
        try (Connection conn = dataSource.getConnection()) {
            ...
            assertThat(orderColumns).contains("id", "customer_id", "status", "subtotal",
                    "discount", "total", "payment_transaction_id", "created_at", "notes",
