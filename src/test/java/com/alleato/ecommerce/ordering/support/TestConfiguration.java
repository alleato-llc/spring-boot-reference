 ...
 *
 * <p><b>AWS SDK test clients</b> — test implementations of {@link SqsClient},
 * {@link SnsClient}, and {@link S3Client} that are injected into the real service
 * implementations (SqsFulfillmentClient, SnsNotificationClient, S3DocumentClient).
 * This means the real service code is exercised in tests, but the actual AWS API
 * calls are intercepted, validated, and recorded by the test clients.</p>
 *
 * <p><b>Payment client test double</b> — Stripe has no AWS SDK dependency, so
 * {@link TestPaymentClient} replaces the client interface directly.</p>
 */
...
public class TestConfiguration {
    ...
    public TestSnsClient testSnsClient() {
        return new TestSnsClient(Set.of(
