...
/**
 * Orchestrates the order workflow: 1. Build the order from the request 2. Price it (via
 * PricingCalculator) 3. Persist it 4. Charge payment (via PaymentClient) 5. Reserve inventory (via
 * InventoryClient) 6. Generate invoice (via InvoiceService) 7. Publish notification (via
 * NotificationClient) 8. Enqueue fulfillment job (via FulfillmentClient / SQS)
 *
 * <p>This is the primary target for integration tests — we want to verify that calling the API
 * results in the correct DB state, payment charge, inventory reservation, invoice storage,
 * notification publication, and fulfillment enqueue.
 */
@Service
public class OrderService {
  ...
  public Order createOrder(CreateOrderRequest request) {
    ...
    PaymentResult result =
        paymentClient.charge(order.getCustomerId(), order.getTotal(), idempotencyKey);

    if (result.success()) {
      order.withStatus(OrderStatus.CONFIRMED).withPaymentTransactionId(result.transactionId());
      ...
      try {
        var confirmation =
            inventoryClient.reserveItems(String.valueOf(order.getId()), reservationItems);
        order.withInventoryReservationId(confirmation.reservationId());
      } catch (InsufficientStockException e) {
        throw new OrderingIllegalArgumentException(
            e.getMessage(), Map.of("productId", e.getProductId()));
      }
      ...
      var payload =
          new FulfillmentPayload(order.getId(), order.getCustomerId(), order.getLineItems().size());
